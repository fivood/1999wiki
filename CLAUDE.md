...
虚线反旋），一级分组为贴在 r2 上的**白底黑字条**——**只分布在右上 1/4 弧**（约 -32°..+56°，等间距），每条长轴径向指向骰子；
锚点为**内左边缘中心**（`translate(var(--r2)) translate(0, -50%)`），padding-right 平滑扩张 = 白条**向外伸长**（对应"选中"）；
不再自转（无旋转闪烁），当前组金色；首页（`px-home`）菜单占整页（hub 左中、骰子放大，像素版不渲染右侧板块，经典版/移动端仍见封面）；
选中白条：仅切换 `.act` 触发 CSS padding-right 过渡（无 380ms 吸附旋转），扇形从**该条外端**径向展开——**径向扇形文字菜单**：
每条词条 = 沿半径方向书写的文字射线（环缘小方点+生长射线牵引），角度小幅散开、半径逐层增大，全部**指向骰子圆心**；
角度步长按视口高度自适应压缩（保证半径级联单调、词条不出屏），超 8 条窗口化+滚轮滑动+范围指示；子分组沿父词条射线继续向外
**级联展开二级**（一级同时压暗，>95° 词条整字翻转防倒置）；点词条/背景/Esc 时菜单**收缩成 54px
