...
选中白条时圆环 380ms 快速旋转吸附到该条几乎水平（-14°），再展开**径向扇形文字菜单**：每条词条 = 沿半径方向倾斜书写的
文字射线（环缘小方点+生长射线牵引），逐条角度加深且半径逐层增大；角度步长按视口高度自适应压缩（保证半径级联单调、词条
不出屏），超 8 条窗口化+滚轮滑动+范围指示；子分组沿父词条射线继续向外**级联展开二级**（一级同时压暗，>95° 词条整字翻转
防倒置）；点词条/背景/Esc 时菜单**收缩成 54px
